  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {stats.claimed > 0 && (() => {
        ...
        const outcomePie = items.filter(i => i.value > 0).map(i => ({ name: i.label, value: i.value, color: i.color.replace('bg-', '').includes('green') ? '#22c55e' : i.color.includes('amber') ? '#f59e0b' : i.color.includes('red') ? '#ef4444' : '#94a3b8' }));
        return (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold flex items-center gap-2">
