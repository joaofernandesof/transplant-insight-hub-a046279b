...
export default function NeoAcademyLesson() {
  ...
  const { data: allLessons } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from('neoacademy_lessons')
        .select('id, title, order_index, module_id, video_duration_seconds, lesson_type, is_preview')
        .eq('course_id', lesson!.course_id)
        .eq('is_published', true)
        .order('order_index');
      return data || [];
          ...
          {modules?.map((mod, mi) => {
            ...
            return (
              <div key={mod.id}>
                ...
                {isExpanded && modLessons.map((l, li) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isCurrent && "bg-violet-500/10 border-l-2 border-violet-500",
                    ...
                    >
                      {isDone ? (
                        ...
                        <Play className="h-3.5 w-3.5 text-violet-400 shrink-0" fill="currentColor" />
                      ) : canAccess ? (
        ...
        <div className="flex-1">
          {lesson.lesson_type === 'video' && lesson.video_url ? (
            ...
            <div className="max-w-3xl mx-auto px-6 py-8">
              <div className="prose prose-invert prose-violet max-w-none prose-headings:text-white prose-p:text-zinc-400 prose-strong:text-zinc-200 prose-li:text-zinc-400 prose-blockquote:border-violet-500 prose-blockquote:text-zinc-300 prose-th:text-zinc-300 prose-td:text-zinc-400">
              ...
              <div>
                ...
                {lesson.video_duration_seconds && lesson.video_duration_seconds > 0 && (
                  <div className="flex items-center gap-3 text-xs text-zinc-500">
                    ...
                    <span className="px-1.5 py-0.5 rounded bg-violet-500/10 text-violet-400 font-medium uppercase text-[10px]">
