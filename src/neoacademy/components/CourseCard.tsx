...
const TYPE_COLORS: Record<CardType, string> = {
  course: 'bg-blue-500/80',
  module: 'bg-sky-500/80',
  lesson: 'bg-cyan-500/80',
};
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "hover:scale-[1.03] hover:shadow-2xl hover:shadow-blue-500/10",
    ...
    >
      ...
      <div className={cn(
        "relative overflow-hidden bg-gradient-to-br from-blue-900/40 to-sky-900/40 shrink-0",
        ...
        {thumbnail ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <BookOpen className="h-10 w-10 text-blue-400/50" />
          </div>
        )}
        ...
        {/* Category badge */}
        {category && (
          <div className="absolute top-2 left-2 px-2 py-0.5 rounded-md bg-blue-500/80 text-[10px] font-bold uppercase text-white tracking-wider">
            {category}
          </div>
        )}
      ...
      <div className={cn(
        ...
        variant === 'continue' ? "flex-1 justify-center" : "h-[88px] justify-between"
      )}>
        <h3 className="font-semibold text-white text-sm leading-[1.3] min-h-[2.6em] line-clamp-2" title={title}>
          {title}
        </h3>
