...
const ICON_MAP: Record<string, React.ComponentType<{ className?: string }>> = {
  Stethoscope, Settings, TrendingUp, Star, Megaphone,
  DollarSign, BarChart, Users, Crown, Workflow, Phone,
  Target, Heart, HeartPulse, Syringe, Briefcase, Building,
  PhoneCall: Phone, Calculator: DollarSign, Network: Settings, Scale: Building,
};

export default function NeoTeamStaffRolesTab() {
  ...
  const deptStats = Object.entries(rolesByDepartment).map(([dept, deptRoles]) => ({
    dept, label: departmentLabels[dept] || dept, count: deptRoles.length,
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {deptStats.map(({ dept, label, count, color }) => (
          <Badge 
            key={dept} variant="outline"
            className={cn("cursor-pointer transition-all", filterDepartment === dept && "ring-2 ring-primary")}
        ...
        <CardContent className="pt-4">
          <div className="flex flex-col gap-4 md:flex-row md:items-center">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar por cargo, descrição ou código..." value={searchTerm}
      ...
      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-lg">Lista de Cargos</CardTitle></CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {filteredRoles.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">Nenhum cargo encontrado</TableCell>
                </TableRow>
              ) : (
                filteredRoles.map((role) => {
                  ...
                  return (
                    <TableRow key={role.id}>
                      <TableCell>
                        <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center", deptColor)}>
                      ...
                      <TableCell className="hidden lg:table-cell">
                        <code className="text-xs bg-muted px-2 py-1 rounded">{role.default_route || '/neoteam'}</code>
                      </TableCell>
