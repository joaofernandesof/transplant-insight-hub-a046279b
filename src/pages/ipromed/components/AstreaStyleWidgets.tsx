...
export function IpromedKPIsWidget() {
  const kpis = [
    ...
    { label: "Tempo de resposta", value: "24h", icon: Clock, color: "text-amber-600" },
    { label: "Foco em saúde", value: "100%", icon: Shield, color: "text-emerald-600" },
  ];
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="h-24 bg-muted/50 rounded-lg flex items-end justify-between px-4 py-2">
          {[40, 25, 60, 80, 45, 30, 55].map((height, idx) => (
            <div key={idx} className="flex flex-col items-center gap-1">
              <div
                className="w-4 bg-muted-foreground/20 rounded-t"
                style={{ height: `${height}%` }}
              />
