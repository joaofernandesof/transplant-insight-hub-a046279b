          ...
          <Table>
            <TableHeader>
              <TableRow className={accentBg}>
                <TableHead className="text-[11px] text-left pl-6 min-w-[90px] font-bold text-foreground">Estrutura</TableHead>
            ...
            <TableBody>
              {rows.map((row, idx) => {
                ...
                const discountDisplay = hasDiscount 
                  ? `${row.discount} (${row.discountPercent})`
                  : row.discount === "—" ? "—" : "0";
                return (
                  <TableRow key={idx} className={idx % 2 === 0 ? 'bg-muted/20' : ''}>
                    <TableCell className="py-2 px-2 pl-6 text-left">
