-- Função RPC para excluir lead do Kanban Avivar em cascata
-- Exclui: lead, mensagens, conversas, journeys
-- Preserva: contato (avivar_contacts)
...
BEGIN
  ...
  -- 3. Delete from leads table (if linked by phone)
  DELETE FROM public.leads WHERE phone = v_phone;

  -- 4. Delete patient journeys (by matching phone)
  DELETE FROM public.avivar_patient_journeys
  WHERE patient_phone = v_phone;
  GET DIAGNOSTICS v_deleted_journeys = ROW_COUNT;

  -- 5. Delete avivar_conversas and avivar_mensagens (legacy chat tables)
  DELETE FROM public.avivar_mensagens
  WHERE conversa_id IN (
    SELECT id FROM public.avivar_conversas WHERE numero = v_phone
  );

  DELETE FROM public.avivar_conversas WHERE numero = v_phone;
