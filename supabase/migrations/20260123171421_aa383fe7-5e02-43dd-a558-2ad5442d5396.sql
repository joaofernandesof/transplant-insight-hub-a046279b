...
-- Update validate_exam_answer to support both letter and text answers
...
BEGIN
  ...
  -- Check if answer matches either:
  -- 1. The correct letter (for backward compatibility)
  -- 2. The correct answer text (for new answers)
  v_is_correct := (
    UPPER(TRIM(p_selected_answer)) = UPPER(TRIM(v_correct_answer_letter))
    OR TRIM(p_selected_answer) = TRIM(v_correct_answer_text)
  );
  
  RETURN QUERY SELECT 
    v_is_correct AS is_correct,
    CASE WHEN v_is_correct THEN COALESCE(v_points, 1) ELSE 0 END AS points_earned,
