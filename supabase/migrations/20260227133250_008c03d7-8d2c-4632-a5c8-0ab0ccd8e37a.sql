...
BEGIN
  ...
  -- ======= STRICT ROUND-ROBIN across all states =======
  -- Pick the state that has been released the LEAST today, 
  -- breaking ties randomly. This guarantees even distribution.
  FOR v_candidate IN (
    SELECT 
      q.state,
      q.queued_count,
    ...
    ORDER BY 
      COALESCE(d.released_count, 0) ASC,  -- Least released first (round-robin)
