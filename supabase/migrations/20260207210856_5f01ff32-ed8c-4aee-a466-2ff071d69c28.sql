-- Fix the RPC function to use 'aberta' instead of 'ativa' which violates the check constraint
...
BEGIN
  ...
  IF v_conversa_id IS NULL THEN
    INSERT INTO public.avivar_conversas (user_id, account_id, numero, nome_contato, conversa_id, status) VALUES (p_user_id::text, v_account_id, p_numero, p_nome_contato, p_conversa_id, 'aberta') RETURNING id INTO v_conversa_id;
