-- Fix: set default webhook_url so it's never null
ALTER TABLE public.lead_webhook_outbox 
  ALTER COLUMN webhook_url SET DEFAULT 'https://n8n-n8n-start.bym1io.easypanel.host/webhook/lead';

-- Also allow NULL temporarily won't work well, better to fix the RPC
-- Update the RPC to pass the actual URL instead of NULL
...
BEGIN
  ...
  -- Webhook outbox (now with proper webhook_url instead of NULL)
