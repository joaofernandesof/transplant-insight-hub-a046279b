...
BEGIN
  ...
  -- Get the states of the last 5 released leads to avoid consecutive same-state releases
  SELECT COALESCE(array_agg(state), ARRAY[]::text[])
  ...
  -- ======= PRIORITIZED ROUND-ROBIN with diversity cooldown =======
  FOR v_candidate IN (
    SELECT 
      q.state,
      q.queued_count,
      ...
      CASE WHEN q.state = ANY(v_recent_states) THEN 1 ELSE 0 END as recently_used
    FROM (
    ...
    ORDER BY 
      ...
      CASE WHEN q.state = ANY(v_recent_states) THEN 1 ELSE 0 END ASC,    -- Avoid recently released states
  ...
  -- FALLBACK: any queued lead (ignores diversity if nothing else available)
