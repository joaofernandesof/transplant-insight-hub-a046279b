...
-- 1) Backfill NULL available_at with updated_at for existing available leads
UPDATE public.leads 
...
-- 2) Fix the release function: 
--    a) ORDER BY available_at DESC NULLS LAST (so NULLs don't pollute last-5 query)
--    b) Use city-only filter (not AND with state) to truly prevent same-city releases
...
BEGIN
  ...
  -- FIX: NULLS LAST so leads without available_at don't pollute the "last 5"
  SELECT 
    array_agg(DISTINCT city) FILTER (WHERE city IS NOT NULL),
  ...
  -- Fallback: near licensees but without anti-duplicate (only if nothing found above)
  ...
  IF v_lead_id IS NULL THEN
    ...
    IF array_length(v_licensee_states, 1) > 0 THEN
      ...
      -- Fallback: allow repeated states if no other option
